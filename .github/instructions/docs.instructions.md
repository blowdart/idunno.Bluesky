---
applyTo: '**/*.cs'
description: 'XML documentation comment guidance for C# code.'
---
...
- Use `<remarks>` for additional information, which can include implementation details, usage notes, or any other relevant context.
  - Wrap all text inside `<remarks>` in `<para></para>` elements, using one `<para>` per paragraph.
...
Order the XML documentation tags for a member as follows, omitting any that don't apply:
...
`<param>` and `<exception>` tags must always appear before `<remarks>`. This also applies to types with primary constructors, where the `<param>` tags for the constructor parameters come before `<remarks>`.
