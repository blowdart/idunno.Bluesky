...
a `string` identifying the device the draft was created on, an optional name for the device or client, and, optionally,
...
To create a draft and save it with a single post, you can use the following code:

```c#
// These should consistently identify the device and client the draft was created on
var deviceId = "5c76194c-fc19-4413-ac45-bf851a289459";
...
var createDraftResult = await agent.CreateDraft(draft);
```

To create a draft with multiple posts, you would construct a collection of `DraftPost`s and pass that to the `Draft` constructor:
...
To embed images or videos in a draft the `DraftPost` constructor takes collections of `DraftEmbedImage` and `DraftEmbedVideo` objects.
...
To retrieve a draft, use the `GetDrafts` method, which will return a collection of `DraftView`s. This is a pageable view which you can
present to your user, allowing them to pick which draft to reload into your UI.

If the user edits a draft and wants to resave it as a draft, use the `UpdateDraft` method, which takes a `DraftWithId` instance.
...
a post length limit in UTF-8 bytes, not in characters, so use `GetUtf8Length()` rather than `string.Length`.
For example,

```c#
if (draftPost.Text.GetUtf8Length() > Maximum.PostLengthInBytes ||
