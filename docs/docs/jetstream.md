...
All metrics are prefixed with `idunno.atproto.jetstream` and are simple Counters.

| Metric | Description |
|--------|-------------|
| total.messages | The number of messages read from the jetstream. |
| total.message_decompression_failures | The number of messages that could not be decompressed |
...
| total.connections_opened | The number of jetstream connections that were opened. |
| total.connections_closed |  The number of jetstream connections that were closed
| total.connections_failed| The number of jetstream failures that were encountered during open or close attempts. |

For example to emit metrics with OpenTelemetry, using [Aspire](https://aspire.dev/) as the dashboard
you would put first put your watcher code in a [HostedService](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/hosted-services),
then in your main method configure the host as follows
