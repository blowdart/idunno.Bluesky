...
{
    /// <summary>
    /// Gets the current commit CID and revision of the specified repository. Does not require authentication.
    /// </summary>
    ...
    [UnconditionalSuppressMessage("AOT",
        ...
        Justification = "All types are preserved in the JsonSerializerOptions call to Get().")]
    public static async Task<AtProtoHttpResult<Commit>> GetLatestCommit(
        Did did,
        Uri service,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AtProtoHttpClient<Commit> request = new(loggerFactory) { MaximumResponseSize = maximumResponseSize };

        return await request.Get(
            service: service,
            endpoint: $"/xrpc/com.atproto.sync.getLatestCommit?did={Uri.EscapeDataString(did.Value)}",
            httpClient: httpClient,
