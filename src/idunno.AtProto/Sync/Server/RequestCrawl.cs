...
{
    /// <summary>
    /// Requests that a service, typically a relay, persistently crawls the repositories hosted by the specified host. Does not require authentication.
    /// </summary>
    /// <param name="hostname">The hostname of the service, such as a personal data server, that is requesting to be crawled.</param>
    /// <param name="service">The relay to send the crawl request to.</param>
    ...
    /// </para>
    /// <para>The expected use is for new personal data servers to declare their existence to relays.</para>
    ...
    [UnconditionalSuppressMessage("AOT",
        ...
        Justification = "All types are preserved in the JsonSerializerOptions call to Post().")]
    public static async Task<AtProtoHttpResult<EmptyResponse>> RequestCrawl(
        string hostname,
        Uri service,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await request.Post(
            service: service,
            endpoint: "/xrpc/com.atproto.sync.requestCrawl",
            record: new RequestCrawlRequest(hostname),
            requestHeaders: null,
