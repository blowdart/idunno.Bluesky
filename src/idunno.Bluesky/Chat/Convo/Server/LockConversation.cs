...
{
    /// <summary>
    /// Locks a group conversation so no more content (messages, reactions) can be added to it.
    /// </summary>
    ...
    [UnconditionalSuppressMessage("AOT",
        ...
        Justification = "All types are preserved in the JsonSerializerOptions call to Post().")]
    public static async Task<AtProtoHttpResult<ConversationView>> LockConversation(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        BlueskyHttpClient<LockConversationResponse> client = new(ChatProxy, loggerFactory) { MaximumResponseSize = maximumResponseSize };

        AtProtoHttpResult<LockConversationResponse> result = await client.Post(
            service,
            "/xrpc/chat.bsky.convo.lockConvo",
            new LockConversationRequest(conversationId),
