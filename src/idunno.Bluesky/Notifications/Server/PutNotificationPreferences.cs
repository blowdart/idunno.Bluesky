...
public static partial class BlueskyServer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        BlueskyHttpClient<Notifications.Model.GetPreferencesResponse> request = new(AppViewProxy, loggerFactory) { MaximumResponseSize = maximumResponseSize };

        PutPreferencesV2Request body = new(preferences);

        AtProtoHttpResult<GetPreferencesResponse> response = await request.Post(
            service: service,
            endpoint: "/xrpc/app.bsky.notification.putPreferencesV2",
            body,
